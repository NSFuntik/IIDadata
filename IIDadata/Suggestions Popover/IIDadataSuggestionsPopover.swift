...
public struct IIDadataSuggestsPopover<S: Suggestion>: ViewModifier {
	...
	var idealHeight: CGFloat {
		...
		return textfieldHeight + (UIFont.preferredFont(forTextStyle: .callout).lineHeight.scaled(by: suggestions))
	}
	...
	public init(
		...
		guard let dadataSuggestions = try? DadataSuggestions.shared() else {
			return
		}
	}
...
//		dadata = try
	...
	public func body(content: Content) -> some View {
		content.coordinateSpace(name: nsPopover)
			...
			.background {
				Color.clear
					.matchedGeometryEffect(id: viewID, in: nsPopover, properties: .frame, anchor: .top, isSource: true)
					.offset(x: 0, y: textfieldHeight / 2)
					.frame(height: idealHeight)
			}
			.overlay(alignment: .top) {
				popover().compositingGroup().matchedGeometryEffect(id: viewID, in: nsPopover, properties: .frame, anchor: .top, isSource: false)
					.fixedSize()
					.layoutPriority(1)
					.zIndex(1)
					.transaction { view in
			...
			.task(id: text) { isPopoverPresented = true; getSuggestions(for: text) }
			.focused($isFocused).animation(.interactiveSpring, value: isFocused)
			.onChange(of: isFocused) { isPopoverPresented = $0 }
			.animation(.interactiveSpring, value: isPopoverPresented)
...
//			.safeAreaInset(edge: .top, content: { Spacer().frame(height: 8) })
			...
			.transition(.offset(x: 0, y: -idealHeight).animation(.interactiveSpring))
		}
	}
...
extension IIDadataSuggestsPopover {
	...
	func getAddressSuggestions(for text: String) async throws(IIDadata.IIDadataError) -> [AddressSuggestion] {
		...
		do {
			guard let suggestions = try await DadataSuggestions.shared().suggestAddress(
				text,
				queryType: .address,
				resultsCount: 10,
				language: .ru
	...
	struct ContentView: View {
		...
		static var addressMock: AddressSuggestion {
			.init(
				...
				data: .init(),
				unrestrictedValue: "г. Санкт-Петербург, улица Грибалёвой, 7 к4 с1, кв. \(Int.random(in: 1 ... 333))"
			)
		}
		...
		init() {
			let apiKey = ProcessInfo.processInfo.environment["IIDadataAPIToken"] ?? "abadf779d0525bebb9e16b72a97eabf4f7143292"
		...
		var body: some View {
			...
			.background(.conicGradient(colors: [Color.pink, .accentColor, .teal, .purple, .brown, .mint, .indigo], center: .center, angle: .degrees(.pi)))
			.ignoresSafeArea()
		}
	}
